...
N_LAYERS_RNN = 3
DEVICES = 3
atten_masks=[1,1,1,1,1.5,1,1,1,1]
        ...
        for idx,conf in enumerate(seq_list_score):
            ...
            for i, th in enumerate(th_list):
                ...
                if predicted_labels.squeeze(1).cpu().long() == torch.LongTensor(labels):
                    ...
                    FP[i] += 1
                    FP_list[i].append(str(labels[0])+'_'+str(torch.sigmoid(logits).squeeze(1).to(device)))
                else:
                    FN[i] += 1
                    FN_list[i].append(str(labels[0])+'_'+str(torch.sigmoid(logits).squeeze(1).to(device)))
...
def main():
    ...
    if (".txt" not in file_path) and (".xlsx" not in file_path):
        ...
        return
    model_path = "H:/20250330 figure+code/Figure 6/3. Perdiction for external data (BiGRU, XGBoost, Transformer)/BiGRU/biGRU.pt"
    ...
    if ".xlsx" in file_path:
        txt_ori,valid_idx_list,s_zmy=convert_xlsx_to_txt(file_path,data_row_idx=1,sheet_name="17aa")
    elif ".txt" in file_path:
        valid_idx_list=None
        txt_ori=file_path

    model = biGRU_ATTEN_Classifier(EMBEDDING_DIM, HIDDEN_DIM, 1, n_layer=N_LAYERS_RNN,atten_mask=atten_masks)
    ...
    out_put_file_path=txt_ori.replace(".txt","_test_result.txt")
    ...
    recall_list.append(round(test_acc[0],1))
    for i in range(0, len(thresholds)):
        print('\n-----------------Eval for threshold of {:.2f}-------------------'.format(thresholds[i]))

        print('test_acc,{:.2f}%,recall,{:.2f}%,test_fp,{},test_fn,{}'
...
# python test_10models_hm.py "H:/20250330 figure+code/Figure 6/4. Prediction for identified citrullinated peptides by Iseq-Cit/identified comfirmed 20250417.xlsx"